...
class aksiSetNama(Action):

    def name(self) -> Text:
        return "aksi_set_nama"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:

        var_nama = tracker.latest_message['text']
        dispatcher.utter_message(text=f"Nama yang diinput adalah {var_nama}!")
        return [SlotSet("nama", var_nama)]

class aksiGetNama(Action):

    def name(self) -> Text:
        return "aksi_get_nama"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:

        var_nama = tracker.get_slot("nama")
        if not var_nama:
            dispatcher.utter_message(text="Maaf yah, aku gak ingat nama kamu.")
        else:
            dispatcher.utter_message(text=f"Nama kamu adalah {var_nama}!")
        return []

# class getEmailByUsername(Action):
#
#     def name(self) -> Text:
#         return "get_email_by_username"
#
#     def run(self, dispatcher: CollectingDispatcher,
#             tracker: Tracker,
#             domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
...
#                                                                   slot_name=resource_type_name,
#                                                                   slot_value=resource_type_value)[0]
#         query_results_type = DbQueryingMethods.select_by_slot(conn=conn,
#                                                               slot_name=resource_type_name,
#                                                               slot_value=resource_type_value)
#
...
#                                                                    slot_name=resource_topic_name,
#                                                                    slot_value=resource_topic_value)[0]
#         query_results_topic = DbQueryingMethods.select_by_slot(conn=conn,
#                                                                slot_name=resource_topic_name,
#                                                                slot_value=resource_topic_value)
#
